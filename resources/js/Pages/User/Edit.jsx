...
export default function Edit({ auth, userData, garduList }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        gardu_induk_id: userData.gardu_induk_id || "",
    });
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        put(route("user.update", userData.id), { preserveScroll: true });
    };
        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h2 className="text-base font-semibold text-gray-800 dark:text-gray-200">Edit User</h2>
                        <p className="text-[11px] text-gray-500 dark:text-gray-400">Perbarui data akun user</p>
                    </div>
                </div>
            }
        >
                            ...
                            <form onSubmit={onSubmit} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <TextInput id="name" value={data.name} onChange={(e) => setData("name", e.target.value)} className={inputStyle} isFocused />
                                        <InputError message={errors.name} className="mt-1" />
                                    </div>

                                    <div>
                                        <InputLabel htmlFor="username" value="Username" className={labelStyle} />
                                        <TextInput id="username" value={data.username} onChange={(e) => setData("username", e.target.value)} className={inputStyle} />
                                        <InputError message={errors.username} className="mt-1" />
                                    </div>

                                    <div>
                                        <InputLabel htmlFor="email" value="Email" className={labelStyle} />
                                        <TextInput id="email" type="email" value={data.email} onChange={(e) => setData("email", e.target.value)} className={inputStyle} />
                                    ...
                                    <div>
                                        <InputLabel htmlFor="password" value="Password Baru" className={labelStyle} />
                                        <TextInput id="password" type="password" value={data.password} onChange={(e) => setData("password", e.target.value)} className={inputStyle} placeholder="Kosongkan jika tidak diganti" />
                                        <InputError message={errors.password} className="mt-1" />
                                    </div>

                                    <div>
                                        <InputLabel htmlFor="password_confirmation" value="Konfirmasi Password Baru" className={labelStyle} />
                                        <TextInput id="password_confirmation" type="password" value={data.password_confirmation} onChange={(e) => setData("password_confirmation", e.target.value)} className={inputStyle} placeholder="Kosongkan jika tidak diganti" />
                                ...
                                <div className="flex items-center justify-end gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                                    ...
                                    <button type="submit" disabled={processing} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-white bg-gradient-to-r from-cyan-600 to-teal-600 rounded-lg shadow-md hover:from-cyan-700 hover:to-teal-700 transition-all disabled:opacity-50">
                                        <Save className="h-4 w-4" /> Perbarui
                                    </button>
                                </div>
                            </form>
                        </div>
